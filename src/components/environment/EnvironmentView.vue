<template>
  <div style="padding: 20px">
    ...
    <br />
    <br />
    <el-button type="primary" @click="handleReStoreUrl">清空缓存环境</el-button>
  </div>
</template>
  ...
  methods: {
    querySearch(queryString, cb) {
      ...
      var results = queryString
        ? environment.filter(this.createFilter(queryString))
        : environment;
    ...
    handleReStoreUrl() {
      this.axios.get(URL.RETROFIT_RE_STORE_URL).then((resp) => {
        if (resp.data.success) {
          this.$message({
            showClose: true,
            message: "清空成功",
