...
import UserRouterView from "../user_router/UserRouterView"
    ...
    if (this.$store.state.deviceInfoGroups.length === 0) {
      this.axios.get(URL.GET_DEVICE_INFO).then((resp) => {
        if (resp.data.success) {
          ...
          const routerNavigation = resp.data.data.routerNavigation;
          this.$store.state.routerNavigation = routerNavigation;
          for (var i = 0; i < routerNavigation.length; i++) {
            this.$router.addRoutes([{ path: "/userRouter-" + i, component: UserRouterView }]);
            this.$routes.push({ path: "/userRouter-" + i, component: UserRouterView });
          }
